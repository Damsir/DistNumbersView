...
#import "DistNumbersView.h"
#import "DistNumbersCollectionCell.h"
#import "DistNumbersCollectionHeaderView.h"
#import "NSString+DistNumbers.h"
#import "DistNumbersViewMarcos.h"
#import <objc/runtime.h>

const CGFloat DistNumbersViewAutomaticDimension = 1646.1646;

NSString * const CellReuseIdentifer = @"DistNumbersViewCollectionCell";
NSString * const HeaderReuseIdentifer = @"DistNumbersViewCollectionHeader";

@interface DistNumbersView () <UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, UIScrollViewDelegate>
...
@implementation DistNumbersView
...
- (void)setupVars {
    ...
    _rowSeparatorStyle = DistNumbersSeparatorStyleDotted;
    _columnSeparatorStyle = DistNumbersSeparatorStyleSolid;
}
...
- (void)caculateWidths {
    ...
    for (NSInteger column = 0; column < _columns; column ++) {
        ...
        if ([_delegate respondsToSelector:@selector(numbersView:widthForColumn:)]) {
            ...
            if (columnWidth == DistNumbersViewAutomaticDimension) {
                needCaculate = YES;
            }
        } else {
            needCaculate = YES;
        ...
        if (needCaculate) {
            ...
            for (NSInteger row = 0; row < _rows; row ++) {
                ...
                CGFloat itemWidth = 0;
                if (string.length) {
                    CGSize stringSize = [string dist_sizeWithFont:[UIFont systemFontOfSize:kDefaultFontSize] constraint:CGSizeMake(_itemMaxWidth, MAXFLOAT)];
...
- (void)updateUI {
    ...
    if (_isFreezeFirstRow) {
        ...
        _headerFreezeCollectionView.frame = CGRectMake(0, 0, _freezeColumnsWidth , _heightForFirstRow);
        
        _bodyFreezeCollectionView.frame = CGRectMake(0, _heightForFirstRow, _freezeColumnsWidth, height - _heightForFirstRow);
        
        _headerSlideCollectionView.frame = CGRectMake(0, 0, _slideColumnWidth, _heightForFirstRow);
        
        _bodySlideCollectionView.frame = CGRectMake(0, _heightForFirstRow, _slideColumnWidth, height - _heightForFirstRow);
        
    } else {
        
        _bodyFreezeCollectionView.frame = CGRectMake(0, 0, _freezeColumnsWidth, height);
        
        _bodySlideCollectionView.frame = CGRectMake(0, 0, _slideColumnWidth, height);
    }
    
    _slideScrollView.frame = CGRectMake(_freezeColumnsWidth, 0, width - _freezeColumnsWidth, height);
...
- (UICollectionView *)initializeCollectionView {
    ...
    c.dataSource = self;
    c.delegate = self;
    [c registerClass:[DistNumbersCollectionCell class] forCellWithReuseIdentifier:CellReuseIdentifer];
    [c registerClass:[DistNumbersCollectionHeaderView class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:HeaderReuseIdentifer];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!cell) {
        ...
        ((DistNumbersCollectionCell *)cell).textHorizontalMargin = _textHorizontalMargin;
        
        ((DistNumbersCollectionCell *)cell).attributedString = [_dataSource numbersView:self attributedStringForItemAtIndexPath:convertedIndexPath];
    }
    return cell;
}
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([kind isEqualToString:UICollectionElementKindSectionHeader]) {
        DistNumbersCollectionHeaderView *headerView = [collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:HeaderReuseIdentifer forIndexPath:indexPath];
...
- (void)setColumnSeparatorStyle:(DistNumbersSeparatorStyle)columnSeparatorStyle {
    if (_columnSeparatorStyle != columnSeparatorStyle) {
        ...
        if (columnSeparatorStyle == DistNumbersSeparatorStyleNone) {
            _slideColumnSeparatorLayer.hidden = YES;
            _freezeColumnSeparatorLayer.hidden = YES;
        } else if (columnSeparatorStyle == DistNumbersSeparatorStyleDotted) {
            _slideColumnSeparatorLayer.lineDashPattern = @[@2];
            _freezeColumnSeparatorLayer.lineDashPattern = @[@2];
        } else if (columnSeparatorStyle == DistNumbersSeparatorStyleSolid){
...
@implementation NSIndexPath (DistNumbersView)
